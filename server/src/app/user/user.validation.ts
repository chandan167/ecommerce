...
export class UserValidation {
    public createSchema: ValidationChain[] = [
        ...
        body('email').trim().notEmpty().withMessage('Email is required').bail().normalizeEmail().isEmail().withMessage('Enter valid email-id').bail()
            .custom(async email => {
                const user = await userService.findByEmail(email)
                if (user) return Promise.reject('E-mail already in use');
                return true
            }),
        body('phone').trim().optional({ nullable: true, checkFalsy: true }).bail().custom(phone => {
            if (phone) {
    ...
    public findByIdsSchema: ValidationChain[] = [
        ...
        body('ids.*').trim().notEmpty().withMessage('Id is required').bail().custom(id => {
            return checkMongooseId(id)
        })
    ]

    public findByIdSchema: ValidationChain[] = [
        body('id').trim().notEmpty().withMessage('Id is required').bail().custom(id => {
            return checkMongooseId(id)
        })
    ]
}